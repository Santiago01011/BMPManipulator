    ...
    if (pointerData.state == CLAY_POINTER_DATA_PRESSED_THIS_FRAME)
    {
        ...
        case ACTION_ROTATE_RIGHT:
            if (!originalImageBMP || !originalImageBMP->pixels) {
                puts("No image loaded to rotate right.");
                return;
            }
        ...
        case ACTION_ROTATE_LEFT:
            if (!originalImageBMP || !originalImageBMP->pixels) {
                puts("No image loaded to rotate left.");
                return;
            }
            backUpImage = (unsigned char *)malloc(originalImageBMP->metadata.height * calculatePitch(&originalImageBMP->metadata));
            if (!backUpImage)
        ...
        case ACTION_CHANGE_R:
            if (!originalImageBMP || !originalImageBMP->pixels) {
                puts("No image loaded to change red value.");
                return;
            }
            backUpImage = (unsigned char *)malloc(originalImageBMP->metadata.height * calculatePitch(&originalImageBMP->metadata));
            if (!backUpImage)
        ...
        case ACTION_CHANGE_G:
            if (!originalImageBMP || !originalImageBMP->pixels) {
                puts("No image loaded to change green value.");
                return;
            }
            backUpImage = (unsigned char *)malloc(originalImageBMP->metadata.height * calculatePitch(&originalImageBMP->metadata));
            if (!backUpImage)
        ...
        case ACTION_CHANGE_B:
            if (!originalImageBMP || !originalImageBMP->pixels) {
                puts("No image loaded to change blue value.");
                return;
            }
            backUpImage = (unsigned char *)malloc(originalImageBMP->metadata.height * calculatePitch(&originalImageBMP->metadata));
            if (!backUpImage)
            {
                puts("Error allocating memory to change blue value");
                return;
            }
            memcpy(backUpImage, (void *)originalImageBMP->pixels, originalImageBMP->metadata.height * calculatePitch(&originalImageBMP->metadata));
        ...
        case ACTION_GRAYSCALE:
            if (!originalImageBMP || !originalImageBMP->pixels) {
                puts("No image loaded to grayscale.");
                return;
            }
            backUpImage = (unsigned char *)malloc(originalImageBMP->metadata.height * calculatePitch(&originalImageBMP->metadata));
            if (!backUpImage)
...
void RenderCentralPanel()
{
    ...
    { /*Here we set the layout for display the current image*/
        ...
        /*Image*/
        CLAY({.layout = {.childAlignment = {.x = CLAY_ALIGN_X_CENTER, .y = CLAY_ALIGN_Y_CENTER}, .sizing = {.width = CLAY_SIZING_FIXED(scaledDimentions.width), .height = CLAY_SIZING_FIXED(scaledDimentions.height)}}, .image = {.imageData = editing_image}})
        {
            if (!originalImageBMP || !originalImageBMP->pixels)
            {
                CLAY_TEXT(CLAY_STRING("NO IMAGE TO DISPLAY"), CLAY_TEXT_CONFIG({.fontId = 0, .fontSize = 42, .textColor = {255, 255, 255, 255}}));
            }
        }
    }
}
...
void RenderEditButtons()
{
    ...
    CLAY({.id = CLAY_ID("EditsButtons"), .backgroundColor = BackgroundColor, .layout = {.sizing = {.height = CLAY_SIZING_FIXED(BOX_H), .width = CLAY_SIZING_GROW(0)}, .padding = CLAY_PADDING_ALL(BOX_PADD), .childGap = BOX_PADD, .childAlignment = {.x = CLAY_ALIGN_X_CENTER, .y = CLAY_ALIGN_Y_CENTER}}, .cornerRadius = CLAY_CORNER_RADIUS(8)})
    {
        ...
        CLAY(rotate_btn)
        {
            ...
            Clay_OnHover(HandleButtonClick, ACTION_TOGGLE_EDIT_MENU);
            CLAY_TEXT(CLAY_STRING("Rotate"), CLAY_TEXT_CONFIG({.fontId = FontIdBody16, .fontSize = 12, .textColor = ColorWhite}));
            ...
            if (rotMenuVisible)
            {
                CLAY({.id = CLAY_ID("rot_menu"), .layout = {.layoutDirection = CLAY_TOP_TO_BOTTOM}, .floating = {.attachTo = CLAY_ATTACH_TO_PARENT, .attachPoints = {.element = CLAY_ATTACH_POINT_RIGHT_BOTTOM, .parent = CLAY_ATTACH_POINT_RIGHT_TOP}}})
                {
                    ...
                    {
                        Clay_OnHover(HandleButtonClick, ACTION_ROTATE_RIGHT);
                        CLAY_TEXT(CLAY_STRING("Right"), CLAY_TEXT_CONFIG({.fontId = FontIdBody16, .fontSize = 12, .textColor = ColorWhite}));
                    }
                    ...
                    {
                        Clay_OnHover(HandleButtonClick, ACTION_ROTATE_LEFT);
                        CLAY_TEXT(CLAY_STRING("Left"), CLAY_TEXT_CONFIG({.fontId = FontIdBody16, .fontSize = 12, .textColor = ColorWhite}));
                    }
                }
            }
        }
        ...
        CLAY(RGB_btn)
        {
            ...
            // Button text and effects
            Clay_OnHover(HandleButtonClick, ACTION_TOGGLE_EDIT_MENU);
            CLAY_TEXT(CLAY_STRING("RGB"), CLAY_TEXT_CONFIG({.fontId = FontIdBody16, .fontSize = 12, .textColor = ColorWhite}));
            ...
            if (rgbMenuVisible)
            {
                CLAY({.id = CLAY_ID("rgb_menu"), .layout = {.layoutDirection = CLAY_TOP_TO_BOTTOM}, .floating = {.attachTo = CLAY_ATTACH_TO_PARENT, .attachPoints = {.element = CLAY_ATTACH_POINT_RIGHT_BOTTOM, .parent = CLAY_ATTACH_POINT_RIGHT_TOP}}})
                {
                    ...
                    {
                        Clay_OnHover(HandleButtonClick, ACTION_CHANGE_R);
                        CLAY_TEXT(CLAY_STRING("Red"), CLAY_TEXT_CONFIG({.fontId = FontIdBody16, .fontSize = 12, .textColor = ColorWhite}));
                    }
                    ...
                    {
                        Clay_OnHover(HandleButtonClick, ACTION_CHANGE_G);
                        CLAY_TEXT(CLAY_STRING("Green"), CLAY_TEXT_CONFIG({.fontId = FontIdBody16, .fontSize = 12, .textColor = ColorWhite}));
                    }
                    ...
                    {
                        Clay_OnHover(HandleButtonClick, ACTION_CHANGE_B);
                        CLAY_TEXT(CLAY_STRING("Blue"), CLAY_TEXT_CONFIG({.fontId = FontIdBody16, .fontSize = 12, .textColor = ColorWhite}));
                    }
                    ...
                    {
                        Clay_OnHover(HandleButtonClick, ACTION_GRAYSCALE);
                        CLAY_TEXT(CLAY_STRING("Grey"), CLAY_TEXT_CONFIG({.fontId = FontIdBody16, .fontSize = 12, .textColor = ColorWhite}));
                    }
                }
            }
        }
    ...
    CLAY({.id = CLAY_ID("OuterContainer"), .backgroundColor = FullBackgroundColor, .layout = {.layoutDirection = CLAY_TOP_TO_BOTTOM, .sizing = layoutExpand, .padding = {5, 5, 5, 5}, .childGap = 5}})
    {
        ...
        { // Child header butons
            ...
            CLAY({.id = CLAY_ID("file_btn"), .layout = {.padding = {BOX_PADD, BOX_PADD, 0, 0}, .sizing = {.height = BTN_H, .width = BTN_W}, .childAlignment = {.x = CLAY_ALIGN_X_CENTER, .y = CLAY_ALIGN_Y_CENTER}}, .backgroundColor = Clay_Hovered() ? ButtonHoverColor : ButtonColor, .cornerRadius = CLAY_CORNER_RADIUS(6), .border = {.width = {1, 1, 1, 1}, .color = !Clay_Hovered() ? FullBackgroundColor : ColorWhite}})
            {
                ...
                Clay_OnHover(HandleButtonClick, ACTION_OPEN_FILE_MENU);
                CLAY_TEXT(CLAY_STRING("File"), CLAY_TEXT_CONFIG({.fontId = FontIdBody16, .fontSize = 12, .textColor = ColorWhite}));
                ...
                if (fileMenuVisible || isFileMenuFocused)
                {
                    ...
                    CLAY({.id = CLAY_ID("file_menu"), .floating = {.attachTo = CLAY_ATTACH_TO_PARENT, .attachPoints = {.parent = CLAY_ATTACH_POINT_LEFT_BOTTOM}}})
                    {
                        CLAY({.layout = {.layoutDirection = CLAY_TOP_TO_BOTTOM, .sizing = {.height = CLAY_SIZING_GROW(0), .width = CLAY_SIZING_FIXED(BTN_W)}}})
                        {
                            ...
                            {
                                // Button text and efects
                                Clay_OnHover(HandleButtonClick, ACTION_OPEN_FILE);
                                CLAY_TEXT(CLAY_STRING("Open"), CLAY_TEXT_CONFIG({.fontId = FontIdBody16, .fontSize = 12, .textColor = ColorWhite}));
                                if (isFilePathInputFocused)
                                {
                            ...
                            {
                                Clay_OnHover(HandleButtonClick, ACTION_NONE);
                                CLAY_TEXT(CLAY_STRING("Close"), CLAY_TEXT_CONFIG({.fontId = FontIdBody16, .fontSize = 12, .textColor = ColorWhite}));
                            }
                        }
                    }
                }
            ...
            {
                Clay_OnHover(HandleButtonClick, ACTION_OPEN_EDIT_MENU);
                CLAY_TEXT(CLAY_STRING("Edit"), CLAY_TEXT_CONFIG({.fontId = FontIdBody16, .fontSize = 12, .textColor = ColorWhite}));
            }
            /* Drop Down Edit Menu */
        }
...
static DisplayImgDimensions _calculateDisplayImgDimentions()
{
    ...
    if (originalImageBMP->angle % 180 != 0)
    { // swap the available space if image is rotated vertically, to adjust layout
